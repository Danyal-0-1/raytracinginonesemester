...
CSV="${OUT_DIR}/timing_cpu.csv"
BATCH_TXT="${OUT_DIR}/timing_cpu_batch.txt"

mkdir -p "${OUT_DIR}"
rm -f "${CSV}" "${BATCH_TXT}"

cmake -S "${ROOT_DIR}" -B "${BUILD_DIR}" -DENABLE_GPU=OFF
...
for preset in "${PRESETS[@]}"; do
  ...
  for spp in "${SPPS[@]}"; do
    out="${OUT_DIR}/cpu_env_${preset}_spp${spp}.png"
    echo "[CPU] scene=${preset} spp=${spp} -> ${out}"
...
for spp in "${SPPS[@]}"; do
  ...
  out="${OUT_DIR}/cpu_pointlight_spp${spp}.png"
  echo "[CPU] scene=pointlight spp=${spp} -> ${out}"
  "${BIN}" "${scene}" --spp "${spp}" --output "${out}" --timing-csv "${CSV}"
done

for spp in "${SPPS[@]}"; do
  out="${OUT_DIR}/cpu_reference_midday_spp${spp}.png"
  echo "[CPU] scene=reference_midday spp=${spp} -> ${out}"
...
echo "cpu,batch_total,batch_total,-1,0,0,0,${batch_seconds}" >> "${BATCH_TXT}"
echo "cpu,batch_total,batch_total,-1,0,0,0,${batch_seconds}" >> "${CSV}"
echo "CPU total batch render time: ${batch_seconds} seconds"
